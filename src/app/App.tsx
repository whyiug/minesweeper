...
export default function App() {
  ...
  return <div className={`app ${motion ? 'with-motion' : 'reduced-motion'}`}>
    <header className="site-header"><div className="brand" aria-label="轻扫雷"><Crystal/><span>轻扫雷<small>LIGHT MINES</small></span></div><p className="tagline">一局刚刚好的专注。</p><nav className="header-actions" aria-label="工具"><button className="icon-button" aria-label="玩法帮助" onClick={() => setPanel('help')}><Icon name="help"/></button><button className={`icon-button ${settings.sound ? 'active' : ''}`} aria-label={settings.sound ? '关闭声音' : '开启声音'} aria-pressed={settings.sound} onClick={toggleSound}><Icon name={settings.sound ? 'sound' : 'mute'}/></button><button className="icon-button" aria-label="打开设置" onClick={() => setPanel('settings')}><Icon name="settings"/></button></nav></header>
    <main>
      <div className="difficulty-bar"><div className="section-label"><span className="tiny-diamond"/>标准扫雷</div><div className="difficulty-options" aria-label="选择难度">{Object.entries(DIFFICULTIES).map(([id, item], index) => <button key={id} className={`difficulty-option ${id === config.difficulty ? 'selected' : ''}`} aria-pressed={id === config.difficulty} onClick={() => choose(id as Difficulty, config.chordMode)}><span className="difficulty-dots" aria-hidden="true">{Array.from({ length: index + 1 }, (_, i) => <i key={i}/>)}</span><strong>{item.name}</strong><small>{item.cols} × {item.rows}<span> · {item.mineCount} 雷</span></small></button>)}</div></div>
